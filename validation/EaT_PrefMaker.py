    ...
    def __init__(self, dn, maxEvts, trainFrac, isBkg):
        ...
        for filename in os.listdir(dn):
            ...
            for event in tree:
                ...
                if isBkg:
                    EcalRecHits = event.EcalRecHits_eat
                    HcalRecHits = event.HcalRecHits_eat
                    SimParticles = event.SimParticles_eat
                    TargetScoringPlaneHits = event.TargetScoringPlaneHits_eat
                    
                    
                else:
                    EcalRecHits = event.EcalRecHits_eat_vis
                    HcalRecHits = event.HcalRecHits_eat_vis
                    SimParticles = event.SimParticles_eat_vis
                    TargetScoringPlaneHits = event.TargetScoringPlaneHits_eat_vis
                ...
                if Eupstream < 3160 and E_hcal > 4840: #not sure if this is the proper place for this check
                    ...
                    for hit in EcalRecHits:
                        z = hit.getZPos()
                        if not z in ecal_layershit:
                            ecal_layershit.append(z)
                    ...
                    for hit in HcalRecHits:
                        if hit.getZPos() >= 870:
                            ...
                            shower_dist += math.sqrt( (x*x) + (y*y) + (z-z_start)**2 )*energy

                            xstd += energy*(x-xmean)**2
                            ystd += energy*(y-ymean)**2
                            zstd += energy*(z-zmean)**2
                            rstd += energy*(r-rmean)**2
                    ...
                    evt.append(Etot) #10
                    evt.append(len(ecal_layershit)) #11
                    evt.append(rstd) #12
                    evt.append(shower_dist) #13
...
if __name__ == '__main__':
    ...
    parser.add_option('--max_evt', dest='max_evt', type='int', default=1800000, help='Max Events to load')
    parser.add_option('--out_dir', dest='out_dir', default='sfs/qumulo/qhome/wer2ct/LDMX/ldmx-analysis/EaT_bdt/weights/', help='Output directory')
    parser.add_option('--out_name', dest='out_name', default='bdt_eat_v2', help='Output Pickle Name')
    parser.add_option('--bdt_path', dest='bdt_path', default='/sfs/qumulo/qhome/wer2ct/LDMX/ldmx-analysis/EaT_bdt/weights/bdt_eat_v2_0/bdt_eat_v2_0_weights.pkl', help='BDT model to load in')
    ...
    parser.add_option('--bkg_dir', dest='bkg_dir', default='/project/hep_aag/ldmx/background/PN_EaT/8gev_batch_1/', help='name of background file directory')
    parser.add_option('--sig_dir', dest='sig_dir', default='/standard/ldmxuva/EaT_signal_8gev/events/', help='name of signal file directory')

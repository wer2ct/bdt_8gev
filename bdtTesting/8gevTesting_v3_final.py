...
class sampleContainer:
    def __init__(self, dn, EDPath, isBkg, bdt):
        ...
        h9 = TH1F("passingHcalDisc","passingHcalDisc",100,0,1) 
            ...
            for event in tree:
                ...
                if Eupstream < 3160 and EHcal > 4840: #Hcal energy requirement
                    ...
                    if recoil_E < 2400: #recoil tracker veto
                        ...
                        if bdt.predict(xgb.DMatrix(np.vstack((evt,np.zeros_like(evt))),np.zeros(2)))[0] >= 0.9997924: #change
                            hcaldisc = bdt.predict(xgb.DMatrix(np.vstack((evt,np.zeros_like(evt))),np.zeros(2)))[0]
                            h4.Fill(decayz)
...
if __name__ == '__main__':
    ...
    parser.add_option('--bdt_path', dest='bdt_path', default='/sfs/qumulo/qhome/wer2ct/LDMX/ldmx-analysis/bdt_8gev/weights/bdt_v2_fixed_3/bdt_v2_fixed_3_weights.pkl', help='BDT model to use') #update weights
